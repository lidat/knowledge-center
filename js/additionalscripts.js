...
$(document).scroll(function onScroll(event) {
  ...
  $(".scrollbarlist a").each(function() {
    // fpr each link in sidebar
    var currLink = $(this);
    ...
    if (
      ...
      // if the position of the link is below the head of the page
...
$("#loginbtn").click(function(event) {
  ...
  $.get(
    "https://api.liveperson.net/api/account/" +
      $(actnum).val() +
      "/service/agentVep/baseURI.json?version=1.0"
  ).done(domainResponse => {
    var domain = domainResponse.baseURI;
    //the actual post method to the Login API using the values from the form
    var posting = $.post(
      `https://${domain}/api/account/${actnum}/login?v=1.3`,
      { username: user, password: pass }
    );
    posting
      .done(() => {
        //when posting is done, trigger this function
